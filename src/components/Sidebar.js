  ...
  render() {
    return (
      <>
          ...
          <div onClick={() => this.props.toggleSidebar()} className={"fixed z-20 inset-0 bg-black opacity-50 transition-opacity lg:hidden " + (this.props.sidebarIsShown ? 'block' : 'hidden')}></div>
          <div className={"fixed z-30 inset-y-0 left-0 w-64 transition duration-300 transform bg-gray-900 overflow-y-auto lg:translate-x-0 lg:static lg:inset-0 " + (this.props.sidebarIsShown ? "translate-x-0 ease-out3" : "-translate-x-full ease-in")}>
...
let LinkItem = ({page, location}) => (
  <li>
    <Link to={page.path} className={"flex items-center mt-4 py-2 px-6  " + (location.pathname === page.path ? " bg-gray-700 bg-opacity-25 text-gray-100" : " text-gray-500 hover:bg-gray-700 hover:bg-opacity-25 hover:text-gray-100")}>
